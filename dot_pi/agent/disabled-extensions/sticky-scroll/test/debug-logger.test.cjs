...
const jiti = createJiti(path.join(__dirname, "debug-logger.test.cjs"), {
  interopDefault: true,
  virtualModules: {
    "@earendil-works/pi-coding-agent": {
      CONFIG_DIR_NAME: ".pi",
      getAgentDir: () => path.join(tmpdir(), "sticky-scroll-agent"),
    },
  },
});
...
async function withTempRoot(run) {
  const root = mkdtempSync(path.join(tmpdir(), "sticky-scroll-debug-"));
  try {
    await run(root);
  } finally {
...
test("enabled debug logger writes on flush and redacts secret fields", async () => {
  await withTempRoot(async (root) => {
    ...
    assert.match(logContent, /"extension":"sticky-scroll"/);
