...
type TuiWithStopPatch = TUI & {
  __stickyScrollStopPatch?: TuiStopPatch;
};
...
const CTRL_PAGE_UP_PATTERN = /^\x1b\[5;5~$/;
const CTRL_PAGE_DOWN_PATTERN = /^\x1b\[6;5~$/;
...
function installStopPatch(tui: TUI): void {
  ...
  if (patchedTui.__stickyScrollStopPatch || typeof patchedTui.stop !== "function") {
    return;
  }

  const originalStop = patchedTui.stop;
  patchedTui.__stickyScrollStopPatch = { originalStop };
  patchedTui.stop = function stickyScrollStopPatch(this: TUI): void {
...
function restoreStopPatch(tui: TUI): void {
  const patchedTui = tui as TuiWithStopPatch;
  const patch = patchedTui.__stickyScrollStopPatch;
  if (!patch) {
    return;
  }

  patchedTui.stop = patch.originalStop;
  delete patchedTui.__stickyScrollStopPatch;
}
...
export function getKeyboardScrollRows(
  data: string,
  pageRows: number,
  options: { allowPlainHomeEnd?: boolean; allowPlainPageKeys?: boolean } = {},
): number | undefined {
  const rows = Math.max(1, Math.floor(pageRows));

  // Ctrl+PageUp/PageDown are explicit history-scroll shortcuts. Plain page keys
  // remain available to the editor whenever it contains text.
  if (matchesKey(data, "ctrl+pageUp") || CTRL_PAGE_UP_PATTERN.test(data)) {
    return -rows;
  }

  if (matchesKey(data, "ctrl+pageDown") || CTRL_PAGE_DOWN_PATTERN.test(data)) {
    return rows;
  }

  if (options.allowPlainPageKeys === true && matchesKey(data, "pageUp")) {
    return -rows;
  }

  if (options.allowPlainPageKeys === true && matchesKey(data, "pageDown")) {
    return rows;
  }
