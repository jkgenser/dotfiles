...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			'Use "scout" or "scout-gemini" only for read-only static codebase reconnaissance with read/grep/find/ls; never delegate shell commands, SQL/database operations, Docker, or other infrastructure/runtime actions to them—the main agent must perform those directly. Use "scout-gemini" when a Gemini second opinion or parallel independent reconnaissance is useful. Use "worker-lite" for straightforward bounded implementation and "worker" for nontrivial or risky implementation.',
			'"scout" uses the direct DeepSeek API at high reasoning effort; "scout-gemini" uses Gemini 3.6 Flash through Google Vertex at high reasoning effort; "worker-lite" uses direct DeepSeek Pro at max; "worker" uses Terra with optional effort=medium/high/xhigh/max (default max).',
