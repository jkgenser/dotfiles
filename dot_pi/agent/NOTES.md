...
- Subagents are installed from Pi's shipped `examples/extensions/subagent` into `extensions/subagent`, with user agents in `agents/` and workflows in `prompts/`. Current agents all use `openai-codex/gpt-5.5`; later swap `agents/scout.md` to `deepseek/deepseek-v4-flash` after DeepSeek auth/model config is in place.
- Pi/OpenAI Codex Fast Mode is implemented in `extensions/service-tier-priority.ts`: use `/fast [on|off|toggle]` to add `service_tier: "priority"` to real `gpt-5.5`/`gpt-5.4` requests without a fake `gpt-5.5-priority` model alias.
