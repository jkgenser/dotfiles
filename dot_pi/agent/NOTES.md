...
- Subagents are installed from Pi's shipped `examples/extensions/subagent` into `extensions/subagent`, with user agents in `agents/` and workflows in `prompts/`. The active roster is intentionally small and uses fixed native thinking levels: `scout` uses the direct `deepseek` API provider with `deepseek-v4-pro:high`, `worker-lite` uses direct `deepseek/deepseek-v4-pro:max`, and `worker` uses `openai-codex/gpt-5.6-terra:max`. Retired agents and effort profiles live under `archive/agents/` and are not discovered. Planning and review are handled by the main agent.
