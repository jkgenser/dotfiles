...
- Watch doubled Tab/Backspace input in Pi on Alacritty. The shell sets `PI_TUI_KITTY_KEYBOARD_FLAGS=0`, but Pi/pi-tui 0.80.3 no longer reads that env var and hardcodes Kitty keyboard flags to `7`. `extensions/alacritty-key-dedupe.ts` restores the intended behavior by consuming duplicate enhanced Tab/Backspace events that follow raw Tab/Backspace while the env var is `0`.
