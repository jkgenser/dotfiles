...
- Subagents are installed from Pi's shipped `examples/extensions/subagent` into `extensions/subagent`, with user agents in `agents/` and workflows in `prompts/`. `agents/scout.md` uses `deepseek/deepseek-v4-flash:medium`; planner/reviewer use `openai-codex/gpt-5.5:xhigh`; implementation workers are dynamically selected among `worker-medium` (`gpt-5.5:medium`), `worker-high` (`gpt-5.5:high`), and `worker-xhigh` (`gpt-5.5:xhigh`).
