...
- Subagents are installed from Pi's shipped `examples/extensions/subagent` into `extensions/subagent`, with user agents in `agents/` and workflows in `prompts/`. `agents/scout.md` uses `deepseek/deepseek-v4-flash:high`; planner/reviewer use `openai-codex/gpt-5.6-sol:xhigh`, while `reviewer-lite` uses `openai-codex/gpt-5.6-luna:xhigh` for focused, bounded reviews. Implementation tiers are `worker-lite` (`gpt-5.6-luna`) for straightforward, bounded, low-risk work, `worker` (`gpt-5.6-terra`) for nontrivial or moderately risky work, and `worker-max` (`gpt-5.6-sol`) for the broadest, most ambiguous, or highest-risk work. Reasoning intensity remains independent of model tier: `worker-lite` supports `effort: high|xhigh|max` (default `high`), while `worker` and `worker-max` support `effort: medium|high|xhigh` (default `medium`); profiles load lazily from `agents/worker-profiles/<effort>.md`.
