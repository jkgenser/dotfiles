...
- Subagents are installed from Pi's shipped `examples/extensions/subagent` into `extensions/subagent`, with user agents in `agents/` and workflows in `prompts/`. `agents/scout.md` uses `deepseek/deepseek-v4-flash:high`; planner/reviewer use `openai-codex/gpt-5.6-sol:xhigh`. Implementation can use `worker-lite` (`gpt-5.6-terra`) for straightforward, bounded, low-risk work or `worker` (`gpt-5.6-sol`) for complex, broad, ambiguous, or high-risk work. Both independently support dynamic `effort: medium|high|xhigh` and lazily load `agents/worker-profiles/<effort>.md`.
